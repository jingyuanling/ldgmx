...
$(function(){
	...
	function a(){
		if(b()){
			...
			$("#submitId").css({
				color:"#bfbfbf",background:"#f0f0f0",border:"1px solid #f0f0f0"
			})
		}
	}
	function n(i,s){
	...
	function h(s,i){
		...
		$(".delok-w").show();
		$(".delok-w").css({
			top:$(window).scrollTop()+$(window).height()/2-$(".delok-w").height()/2
		});
		var t=setTimeout(function(){
			$(".popup-w").hide();
			$(".delok-w").hide()
	...
	Zepto("#puller").generateRegionList({
		...
		callBack:function(){
			...
			$("#cityNameIgnoreId").val(u);$("#areaNameIgnoreId").val(t);
			$("#townNameIngoreId").val(s);a()
		}
	});
	$(".switch").click(function(){
		...
		pingClick(document.getElementById("cartUse").value,"MNeworderNewAddress_DefaultAddress","","","")
	});
	$(".confirm-del").click(function(){
		...
		$(".confirm-w").show();
		$(".confirm-w").css({
			top:$(window).scrollTop()+$(window).height()/2-$(".confirm-w").height()/2
		});
	...
	$("#submitId").click(function(){
		...
		if(g()){
			...
			if(document.getElementById("address.idProvince").value==84||document.getElementById("provinceNameIgnoreId").value=="钓鱼岛"){
				...
				h("保存并使用成功!",1000);
				setTimeout("$('#editAddressForm').submit()",100);
				pingClick(document.getElementById("cartUse").value,"MNeworderNewAddress_Save","","","")
			}
			return true
		}
		j();
